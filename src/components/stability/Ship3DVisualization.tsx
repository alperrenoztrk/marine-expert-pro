...
export const Ship3DVisualization = () => {
  ...
  const [viewMode, setViewMode] = useState<"photo" | "model">("photo");
  const [photoErrors, setPhotoErrors] = useState<Record<ShipType, boolean>>({
    tanker: false,
    bulk: false,
    container: false,
    roro: false,
  ...
  const activePhoto = shipTypePhotos[shipType];
  const photoAvailable = !photoErrors[shipType];
  ...
  return (
    <Card className="border-primary/20">
      ...
      <CardContent className="space-y-4">
        ...
        <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as "photo" | "model")} className="space-y-2">
          <TabsList className="w-full sm:w-fit">
            <TabsTrigger value="photo">Fotoğraf</TabsTrigger>
            <TabsTrigger value="model">3D Model</TabsTrigger>
          </TabsList>
          <TabsContent value="photo">
            <div className="relative w-full h-[300px] rounded-lg overflow-hidden border border-border/40 bg-slate-950">
              {photoAvailable ? (
                <>
                  <img
                    src={activePhoto.src}
                    alt={`${activeShipType?.label} gemisi fotoğrafı`}
                ...
                <div className="flex h-full items-center justify-center text-xs text-muted-foreground">
                  Fotoğraf yüklenemedi. 3D modele geçebilirsiniz.
                </div>
              )}
            </div>
          </TabsContent>
