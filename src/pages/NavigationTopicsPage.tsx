      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <section className="rounded-2xl border border-border/40 bg-card/80 p-6 backdrop-blur">
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {navigationTopicsContent.map((topic) => (
              <AccordionItem key={topic.id} value={topic.id} className="rounded-xl border border-border/40 bg-background/60 px-4">
                ...
                <AccordionContent>
                  <div className="grid gap-4 pt-3 md:grid-cols-2">
                    {topic.pages.map((page) => (
                      <div
                        key={page.title}
                        className="flex h-full flex-col gap-3 rounded-xl border border-border/40 bg-card/70 p-4 shadow-sm"
                      >
                        <div>
                              ...
                              <div className="space-y-1">
                                ...
                                <ul className="mt-1 space-y-1">
                                  {page.references.map((reference) => (
                                    <li key={`${page.title}-${reference}`} className="flex items-start gap-2">
                  ...
                  <div className="mt-4 rounded-xl border border-border/40 bg-background/60 p-4">
                    ...
                    <ul className="mt-3 space-y-2 text-xs text-muted-foreground">
                      {topic.pages.map((page) => (
                        <li key={`${topic.id}-${page.imageSrc}`} className="flex flex-col gap-1">
                  ...
                  <div className="mt-4 grid gap-4 rounded-xl border border-border/40 bg-background/60 p-4 lg:grid-cols-3">
                    <div className="space-y-2">
                      ...
                      <ul className="space-y-2 text-xs text-muted-foreground">
                        {topic.pages.slice(0, 3).map((page) => (
                          <li key={`${topic.id}-${page.title}-check`} className="flex items-start gap-2">
                            <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-primary" />
