...
export class StabilityCalculationEngine {
  ...
  /**
   * Calculate stability during cargo operations
   */
  static calculateCargoStability(
    vessel: VesselData,
    initialLoading: LoadingCase,
    cargoSequence: Array<{ weight: number; lcg: number; vcg: number; tcg: number; operation: 'load' | 'discharge' }>
  ) {
    const stages = [];
    let currentLoading = { ...initialLoading };
    
    // Initial stage
    const initialAnalysis = this.performCompleteAnalysis(vessel, currentLoading);
    stages.push({
      stage: 0,
      operation: 'initial',
      displacement: initialAnalysis.loadingSummary.total_displacement,
      kg: initialAnalysis.KG_corrected,
      gm: initialAnalysis.GM_t,
      stable: initialAnalysis.GM_t > 0.15,
      warnings: initialAnalysis.warnings
    });
    
    // Process each cargo operation
    for (let i = 0; i < cargoSequence.length; i++) {
      const operation = cargoSequence[i];
      
      if (operation.operation === 'load') {
        currentLoading.items.push({
        ...
        // Find and remove cargo item (simplified - remove last matching weight)
        const index = currentLoading.items.findLastIndex(item => 
          Math.abs(item.weight - operation.weight) < 0.1
        );
        if (index >= 0) {
          currentLoading.items.splice(index, 1);
        }
      }
      
      const analysis = this.performCompleteAnalysis(vessel, currentLoading);
      stages.push({
        stage: i + 1,
        operation: operation.operation,
        displacement: analysis.loadingSummary.total_displacement,
        kg: analysis.KG_corrected,
        gm: analysis.GM_t,
        stable: analysis.GM_t > 0.15,
        warnings: analysis.warnings
      });
    }
    
    // Find critical stage
    const minimumGM = Math.min(...stages.map(s => s.gm));
    const criticalStage = stages.find(s => s.gm === minimumGM)?.stage || -1;
    
    return {
      stages,
      minimumGM,
      criticalStage,
      overallSafe: minimumGM > 0.15
    };
  }
  ...
  static calculateParametricRolling(
    ...
    const rollingPeriod = this.calculateRollingPeriod(vessel, gm);
    
    // Wave period (deep water approximation)
    const wavePeriod = 2 * Math.PI * Math.sqrt(waveLength / this.GRAVITY);
    
    // Parametric rolling occurs at half the rolling period
    const parametricPeriod = rollingPeriod / 2;
    
    // Check for resonance conditions
    const lengthRatio = waveLength / vessel.principal_dimensions.LBP;
    const periodRatio = Math.abs(wavePeriod - parametricPeriod) / parametricPeriod;
    const waveHeightRatio = waveHeight / vessel.principal_dimensions.breadth;
    
    const lengthMatch = lengthRatio >= 0.8 && lengthRatio <= 1.2;
    const periodMatch = periodRatio <= 0.2;
    const significantWave = waveHeightRatio >= 0.1;
    ...
    return {
      ...
      waveHeightRatio,
      susceptible,
      riskLevel: susceptible ? 'HIGH' : (periodMatch || lengthMatch) ? 'MEDIUM' : 'LOW'
    };
  }

  /**
   * Calculate rolling period for vessel
   */
  private static calculateRollingPeriod(vessel: VesselData, gm: number): number {
    const breadth = vessel.principal_dimensions.breadth;
    const radiusOfGyration = breadth * 0.35; // Typical value for merchant vessels
  ...
  static calculateOptimumTrim(
    ...
    const length = vessel.principal_dimensions.LBP;
    const froudeNumber = speed / Math.sqrt(this.GRAVITY * length);
    
    // Optimum trim percentage based on Froude number
    let optimumTrimPercent = 0.005; // Default 0.5% of LBP
    ...
    const optimumTrim = length * optimumTrimPercent;
    
    // Calculate resistance reduction (approximate)
    const resistanceReduction = optimumTrim * 0.02; // 2% per meter of optimum trim
    
    // Calculate recommended drafts
    const meanDraft = displacement / (length * vessel.principal_dimensions.breadth * 
                     vessel.form_coefficients.block * this.SEAWATER_DENSITY);
    ...
    return {
      ...
      resistanceReduction,
      recommendedDrafts: {
        forward: meanDraft - optimumTrim / 2,
        aft: meanDraft + optimumTrim / 2
      }
    };
  }
